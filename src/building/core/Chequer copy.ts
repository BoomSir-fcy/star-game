...
class Chequer extends EventTarget {
  ...
  static path = [
    ...
    Chequer.HEIGHT * Chequer.Y_RATIO + Chequer.OFFSET_Y,
    0 + Chequer.OFFSET_X - 10,
    Chequer.HEIGHT * Chequer.Y_RATIO * 2 + Chequer.OFFSET_Y - 10,
    -Chequer.WIDTH * Chequer.X_RATIO + Chequer.OFFSET_X + -10,
    Chequer.HEIGHT * Chequer.Y_RATIO - 10,
  ];

  // 底色是不规则渲染 所以事件范围也不规则
  // 使用 Graphics 绑定事件 进行hack处理
  createGraphics() {
    console.log(Chequer.path);
    this.graphics.lineStyle(1, 0x4ffffb, 0.7);
    this.graphics.beginFill(0x4ffffb, 0.2);
    this.graphics.drawPolygon(Chequer.path);
