...
class PreSoldier extends EventTarget {
  ...
  init(src: string, width: number, height: number) {
    ...
    this.soldier.container
      .on('pointerdown', (event: InteractionEvent) => {
        ...
        this.dispatchEvent(new CustomEvent('pointerdown', { detail: { soldier: this.copySoldier, event } }));
      })
      .on('pointerup', (event: InteractionEvent) => {
        this?.soldier?.onDragEnd();
        this.dispatchEvent(new CustomEvent('pointerup', { detail: { soldier: this.copySoldier, event } }));

      })
      .on('pointerupoutside', (event: InteractionEvent) => {
        this?.soldier?.onDragEnd();
        this.dispatchEvent(new CustomEvent('pointerupoutside', { detail: { soldier: this.copySoldier, event } }));
      })
      .on('pointermove', (event: InteractionEvent) => {
        this.onDragMove();
        this.dispatchEvent(new CustomEvent('pointermove', { detail: { soldier: this.copySoldier, event } }));

      });

    this.app.stage.addChild(soldier.container);
    console.log(this.soldier.container)
  ...
  onDragMove() {
    if (this.soldier?.dragging && this.copySoldier) {
      ...
      const newPosition = this.copySoldier.dragData?.getLocalPosition(this.soldier.container.parent);
      if (newPosition) {
