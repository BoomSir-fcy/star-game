...
export const getEffectText = (effect?: DescType) => {
  ...
  if (effect === descType.REMOVE_FIRING) return 'Purge'; // '灭火'
  if (effect === descType.ADD_SHIELD) return '护盾';
  if (effect === descType.REMOVE_SHIELD) return '减少护盾';
  if (effect === descType.REMOVE_STOP_MOVE) return 'Unlock';
  if (effect === descType.REMOVE_BOMB) return 'Disarm Bomb';
  ...
  if (effect === descType.IMMUNITY_ICE) return 'Immune Freeze';
  if (effect === descType.IMMUNITY_LOCK_MOVE) return 'Immune Lock up';
  if (effect === descType.IMMUNITY_FIRING) return 'Immune Burning';
  return '未知';
};
...
export const getEffectDescText = (effect?: DescType) => {
  ...
  if (effect === descType.FIRING) return 'Burned, caused';
  if (effect === descType.BOOM) return 'Bomb Exploded, caused';
  if (effect === descType.ICE_START) return '冰冻';
  ...
  if (effect === descType.TERRAIN_FIRING) return 'Terrain Burned, caused';
...
export const getEffectDescTextNew = (effect?: DescType) => {
  if (effect === descType.ATTACK) return 'used General Attack';
  if (effect === descType.STOP_MOVE) return 'use Lock up';
  if (effect === descType.ICE_START) return 'use Freeze Attack';
  if (effect === descType.ICE_END) return '解除冰冻';
  if (effect === descType.ADD_FIRING) return 'add Burn';
  if (effect === descType.FIRING) return '被灼烧';
  if (effect === descType.ADD_BOOM) return 'place Bomb';
  if (effect === descType.BOOM) return '使用炸弹攻击';
  if (effect === descType.REMOVE_FIRING) return 'use Fire Extinguishing';
  if (effect === descType.PURIFY) return 'use Purge';
  if (effect === descType.ADD_SHIELD) return 'add Shield';
  if (effect === descType.REMOVE_SHIELD) return '攻击护盾';
  if (effect === descType.REMOVE_STOP_MOVE) return 'Unlock';
  if (effect === descType.REMOVE_BOMB) return 'Disarm Bomb';
  if (effect === descType.ADD_TERRAIN_FIRING) return 'generate Terrain Burn';
  if (effect === descType.TERRAIN_FIRING) return 'Terrain Burned';
  if (effect === descType.RESTORE) return 'use Treatment';
  if (effect === descType.ATTACK_DODGE) return 'and triggered EVA';
  if (effect === descType.ATTACK_MISS) return 'and missed the target';
  if (effect === descType.IMMUNITY_ICE) return 'Freeze Attack';
  if (effect === descType.IMMUNITY_LOCK_MOVE) return 'Lock up Attack';
  if (effect === descType.IMMUNITY_FIRING) return 'Burn Attack';
  return '未知';
};

export const getEffectDescTypeText = (effect?: DescType) => {
  if (effect === descType.ATTACK) return 'damage';
  if (effect === descType.STOP_MOVE) return 'effect';
  if (effect === descType.ICE_END) return 'effect';
  if (effect === descType.FIRING) return 'damage';
  if (effect === descType.BOOM) return 'damage';
  if (effect === descType.ICE_START) return 'effect';
  if (effect === descType.ADD_FIRING) return 'effect';
  if (effect === descType.ADD_BOOM) return 'effect';
  if (effect === descType.REMOVE_FIRING) return 'effect';
  if (effect === descType.PURIFY) return 'effect';
  if (effect === descType.RESTORE) return 'effect';
  if (effect === descType.ADD_SHIELD) return 'effect';
  if (effect === descType.REMOVE_SHIELD) return '';
  if (effect === descType.REMOVE_BOMB) return '';
  if (effect === descType.REMOVE_STOP_MOVE) return 'effect';
  if (effect === descType.ADD_TERRAIN_FIRING) return 'effect';
  if (effect === descType.TERRAIN_FIRING) return 'damage';
  if (effect === descType.ATTACK_DODGE) return 'effect';
