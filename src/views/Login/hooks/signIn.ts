...
export const useCheckName = () => {
  ...
  const handleCheck = useCallback( async (name) => {
    ...
    return res ? CheckNickNameState.EXACT_NAME : CheckNickNameState.BAD_NAME_WITH_CONTRACT
  }, [contract])
  return {
    handleCheck,
  }
...
export const useRegisterWithDsg = () => {
  ...
  /**
    ...
    */
  const handleRegister = useCallback(async ({ nickname, superior, gender }: RegisterParams) => {
    const tx = await contract.registerWithDsg(nickname, superior, gender);
